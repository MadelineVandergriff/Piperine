...
namespace pip::detail {
    struct Instance : Wrap<VkInstance> {
        static InstanceRef_ create(VkInstanceCreateInfo info);
    };

    struct Pipeline : Wrap<VkPipeline> {};

    struct GLFWSingleton : Singleton<GLFWSingleton> {
        friend struct Singleton<GLFWSingleton>;
        ~GLFWSingleton();
    private:
        GLFWSingleton();
    };
    ...
    struct DescriptorSetRef : PtrRef<VkDescriptorSet> {
